import type { PartDimensions, PartPrintSettings } from '@/types/database'

/**
 * Parsing and validation for a part's physical/print metadata — dimensions,
...
// Column length limits mirrored from the parts table CHECK constraints.
 ...
 */
export function parseDimensions(raw: string): ParseResult<PartDimensions> {
  let parsed: unknown
  try {
  ...
  if (obj.unit !== undefined) result.unit = obj.unit as PartDimensions['unit']
 ...
 */
export function parsePrintSettings(raw: string): ParseResult<PartPrintSettings> {
  let parsed: unknown
  try {
    parsed = JSON.parse(raw)
  } catch {
  ...
  const result: PartPrintSettings = {}
  ...
  if (ps.supports !== undefined) result.supports = ps.supports as PartPrintSettings['supports']
  return { ok: true, data: result }
}

/** Flat metadata form fields, as held by the part-upload form state. */
export interface PartMetadataFormFields {
  material: string
  color: string
...
export interface SerializedPartMetadata {
  material: string
  color: string
  dimensions: string
 ...
 */
export function serializePartMetadata(f: PartMetadataFormFields): SerializedPartMetadata {
